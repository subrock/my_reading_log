...
// GetLastTitle - Takes an api call and returns xml with the GetLastTitle by reader_id. 
...
writeToLog("Starting GetLastTitle.");
...
if (strcmp($api_sign, $apikey) !== 0) {
	...
	writeToLog($query);
        ...
        $lstTitle=mysql_result($result,"entry_title");
	writeToLog($lstTitle);
	$xml = $xml . "<title>".$lstTitle."</title>";
	$xml = $xml . "</entry>";
	writeToLog($xml);
	echo $xml;
	writeToLog("End GetLastTitle.\n\n");
	exit;
} else {
